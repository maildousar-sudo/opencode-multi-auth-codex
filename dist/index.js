            ...
            async loader(getAuth, provider) {
                ...
                const customFetch = async (input, init) => {
                    ...
                    const eligibleCount = Object.values(store.accounts).filter(acc => {
                        const now = Date.now();
                        return (!acc.rateLimitedUntil || acc.rateLimitedUntil < now) && (!acc.modelUnsupportedUntil || acc.modelUnsupportedUntil < now) && (!acc.workspaceDeactivatedUntil || acc.workspaceDeactivatedUntil < now) && !acc.authInvalid && acc.enabled !== false;
                    }).length;
                    ...
                    while (attempt < maxAttempts) {
                        ...
                        if (!rotation) {
                            if (forcePinned && forceState.forcedAlias) {
                                ...
                                if (forced?.rateLimitedUntil && forced.rateLimitedUntil > now) {
                                    return new Response(JSON.stringify({ error: { code: 'RATE_LIMITED', message: `Forced account '${forced.alias}' is rate-limited until ${new Date(forced.rateLimitedUntil).toISOString()}`, details: { alias: forced.alias, rateLimitedUntil: forced.rateLimitedUntil } } }), { status: 429, headers: { 'Content-Type': 'application/json' } });
                                }
                            }
                            return new Response(JSON.stringify({ error: Errors.noEligibleAccounts('No available accounts after filtering') }), { status: 503, headers: { 'Content-Type': 'application/json' } });
                        }
                        ...
                        if (!accountId) {
                            return new Response(JSON.stringify({ error: { code: 'TOKEN_PARSE_ERROR', message: '[multi-auth] Failed to extract accountId from token' } }), { status: 401, headers: { 'Content-Type': 'application/json' } });
                        }
                        ...
                        const payload = { ...body, model: normalizedModel, store: false };
                        ...
                        if (reasoningMatch?.[1]) {
                            payload.reasoning = { ...(payload.reasoning || {}), effort: reasoningMatch[1], summary: payload.reasoning?.summary || 'auto' };
                        }
                        ...
                        try {
                            ...
                            const res = await fetch(url, { method: init?.method || 'POST', headers, body: JSON.stringify(payload) });
                            const limitUpdate = extractRateLimitUpdate(res.headers);
                            const mergedRateLimits = limitUpdate ? mergeRateLimits(account.rateLimits, limitUpdate) : account.rateLimits;
                            if (limitUpdate) {
                                updateAccount(account.alias, { rateLimits: mergedRateLimits });
                            }
                            if (res.status === 401 || res.status === 403) {
                                ...
                                return new Response(JSON.stringify({ error: Errors.maxRetriesExceeded(attempt, Array.from(triedAliases)) }), { status: res.status, headers: { 'Content-Type': 'application/json' } });
                            }
                            if (res.status === 429) {
                                const errorData = await res.clone().json().catch(() => ({}));
                                ...
                                return new Response(JSON.stringify({ error: Errors.maxRetriesExceeded(attempt, Array.from(triedAliases)) }), { status: 429, headers: { 'Content-Type': 'application/json' } });
                            }
                            if (res.status === 402) {
                                const errorData = await res.clone().json().catch(() => null);
                                const errorText = await res.clone().text().catch(() => '');
                                const code = (typeof errorData?.detail?.code === 'string' && errorData.detail.code) || (typeof errorData?.error?.code === 'string' && errorData.error.code) || '';
                                const message = (typeof errorData?.detail?.message === 'string' && errorData.detail.message) || (typeof errorData?.detail === 'string' && errorData.detail) || (typeof errorData?.error?.message === 'string' && errorData.error.message) || (typeof errorData?.message === 'string' && errorData.message) || errorText || '';
                                const isDeactivatedWorkspace = code === 'deactivated_workspace' || message.toLowerCase().includes('deactivated_workspace') || message.toLowerCase().includes('deactivated workspace');
                                if (isDeactivatedWorkspace) {
                                    markWorkspaceDeactivated(account.alias, pluginConfig.workspaceDeactivatedCooldownMs, { error: message || code });
                                    if (attempt < maxAttempts) {
                                        continue;
                                    }
                                    return new Response(JSON.stringify({ error: Errors.maxRetriesExceeded(attempt, Array.from(triedAliases)) }), { status: 402, headers: { 'Content-Type': 'application/json' } });
                                }
                            }
                            if (res.status === 400) {
                                const errorData = await res.clone().json().catch(() => ({}));
                                const message = (typeof errorData?.detail === 'string' && errorData.detail) || (typeof errorData?.error?.message === 'string' && errorData.error.message) || (typeof errorData?.message === 'string' && errorData.message) || '';
                                const isModelUnsupported = typeof message === 'string' && message.toLowerCase().includes('model is not supported') && message.toLowerCase().includes('chatgpt account');
                                if (isModelUnsupported) {
                                    markModelUnsupported(account.alias, pluginConfig.modelUnsupportedCooldownMs, { model: normalizedModel, error: message });
                                    if (attempt < maxAttempts) {
                                        continue;
                                    }
                                    return new Response(JSON.stringify({ error: Errors.maxRetriesExceeded(attempt, Array.from(triedAliases)) }), { status: 400, headers: { 'Content-Type': 'application/json' } });
                                }
                            }
                            if (!res.ok) {
                                return res;
                    ...
                    return new Response(JSON.stringify({ error: Errors.maxRetriesExceeded(attempt, Array.from(triedAliases)) }), { status: 503, headers: { 'Content-Type': 'application/json' } });
                };
                return { apiKey: 'chatgpt-oauth', baseURL: CODEX_BASE_URL, fetch: customFetch };
            },
            methods: [
                {
                    label: 'ChatGPT OAuth (Multi-Account)',
                    type: 'oauth',
                    prompts: [{ type: 'text', key: 'alias', message: 'Account alias (e.g., personal, work)', placeholder: 'personal' }],
                        ...
                        return {
                            ...
                            callback: async () => {
                                try {
                                    ...
                                    return { type: 'success', provider: PROVIDER_ID, refresh: account.refreshToken, access: account.accessToken, expires: account.expiresAt };
                                }
                                catch {
                                    return { type: 'failed' };
                                }
